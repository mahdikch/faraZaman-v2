...
import Ir.co.tfs.farazaman.data.db.model.TrackPoint;
import Ir.co.tfs.farazaman.data.db.model.WayPoint;
...
public class DataHelper {
	 ...
	 */
	public static final String FILE_PROVIDER_AUTHORITY = "Ir.co.tfs.farazaman.fileprovider";
	...
	public static long getActiveTrackId(ContentResolver cr) {
		return getActiveTrackId(cr, null);
	}
	
	/**
	 * Find the active track ID for a specific role, if any.
	 * @param cr  {@link ContentResolver} for query
	 * @param role  Role filter ("driver" or "supervisor"), or null for any role
	 * @return  the active track ID, or -1
	 */
	public static long getActiveTrackId(ContentResolver cr, String role) {
		long currentTrackId = -1;
		String selection = TrackContentProvider.Schema.COL_ACTIVE + " = ?";
		String[] selectionArgs;
		
		if (role != null && !role.isEmpty()) {
			selection += " AND " + TrackContentProvider.Schema.COL_ROLE + " = ?";
			selectionArgs = new String[] {
				Integer.toString(TrackContentProvider.Schema.VAL_TRACK_ACTIVE),
				role
			};
		} else {
			selectionArgs = new String[] {Integer.toString(TrackContentProvider.Schema.VAL_TRACK_ACTIVE)};
		}
		
		Cursor ca = cr.query(
			TrackContentProvider.CONTENT_URI_TRACK,
			new String[] {TrackContentProvider.Schema.TBL_TRACK + "." + TrackContentProvider.Schema.COL_ID + " as " + TrackContentProvider.Schema.COL_ID},
			selection,
			selectionArgs,
			null
		);
